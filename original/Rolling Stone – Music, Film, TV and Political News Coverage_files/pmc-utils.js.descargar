...
Object.assign(pmc, {
	...
	/**
	 * When a deferred script includes an "after" script (added via
	 * `wp_add_inline_script()`), we need to ensure that the "after" script
	 * executes _after_ the deferred script is loaded. Adding the `defer`
	 * attribute to the inline "after" script does not ensure that the "after"
	 * script executes after its parent, so we add an `onload` event to the
	 * parent script, which calls this function with the handle of the parent
	 * script.
	 *
	 * Works in conjunction with modifications made to the script-loader tags in
	 * `\PMC\Core\Inc\Assets::filter_script_loader_tag()`.
	 *
	 * @param {string} scriptHandle Script's registered handle.
	 */
	deferredScriptOnloadCallback: function( scriptHandle ) {
		var element = document.getElementById( scriptHandle + '-js-after' );

		if ( ! element ) {
			return;
		}

		if ( 'text/plain' !== element.getAttribute( 'type' ) ) {
			return;
		}
